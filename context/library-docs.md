...
- Use `@insforge/sdk/ssr` for `createBrowserClient`, `createServerClient`, `createAuthActions`, and `createRefreshAuthRouter`
- Use `@insforge/sdk/ssr/middleware` for `updateSession` in root `proxy.ts`
- Keep `/api/auth/refresh` as the browser refresh endpoint
- Start OAuth in a Server Action with `skipBrowserRedirect: true`
- Exchange `insforge_code` in `/api/auth/callback` and redirect successful users to `/dashboard`
- Store the PKCE `insforge_code_verifier` in an httpOnly cookie with a short max age
